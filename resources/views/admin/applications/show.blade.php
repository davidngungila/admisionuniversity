    ...
    <div class="panel">
        ...
        <div class="panel-body">
            @forelse($application->academicResults as $r)
                <div class="kv" style="margin-bottom:8px"><div class="kv-row"><span class="k">{{ $r->exam_type }} {{ $r->index_number }} ({{ $r->exam_year }})</span><span class="v"><span class="tag {{ $r->is_verified ? (str_ends_with($r->exam_body ?? '', '(BYPASSED)') ? 'tag-gold' : 'tag-green') : 'tag-grey' }}">{{ $r->is_verified ? (str_ends_with($r->exam_body ?? '', '(BYPASSED)') ? 'Verified (BYPASSED)' : 'Auto-verified · '.($r->exam_body ?? 'External')) : 'Pending verification' }}</span> · {{ count($r->results ?? []) }} subjects</span></div></div>
            @empty
