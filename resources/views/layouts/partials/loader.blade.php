...
@php $loaderLogo = \App\Models\Setting::getValue('university_logo'); @endphp
<div id="pageLoader" class="page-loader" aria-hidden="true">
    <div class="loader-logo">
        <div class="loader-ring"></div>
        <div class="loader-mark" style="overflow:hidden;@if($loaderLogo)background:#fff;padding:4px;@endif">@if($loaderLogo)<img src="{{ asset($loaderLogo) }}" style="width:100%;height:100%;object-fit:contain;border-radius:10px;" alt="Logo">@else {{ substr(\App\Models\Setting::getValue('university_acronym','UDOM'),0,1) }} @endif</div>
    </div>
    <div class="loader-text">{{ \App\Models\Setting::getValue('university_name','University OAS') }}</div>
...
(function(){
    ...
    // Hide after full load (with small delay for smooth cycle)
    window.addEventListener('load', ()=> setTimeout(hideLoader, 400));
    // Fallback hide after DOM ready if load already fired
    ...
    // Show on navigation / form submit
    document.addEventListener('click', e=>{
        ...
        if(a && a.href && !a.target && !a.href.startsWith('javascript:') && !a.href.startsWith('#') && !a.hasAttribute('download') && a.origin === location.origin){
            // Don't show for hash or same-page
            if(a.href !== location.href) showLoader();
        }
    });
    document.addEventListener('submit', e=>{
        // If submit was prevented for a confirm modal, don't show loader yet
        if(e.defaultPrevented) return;
        const f = e.target;
        if(f.tagName === 'FORM' && !f.hasAttribute('data-no-loader')){
            // Don't show loader if a confirm modal is currently open
            if(document.querySelector('.modal-backdrop.show')) return;
            showLoader();
        }
    });
    // For programmatic submits via confirmModal (form.submit() doesn't fire submit event)
    ...
    // Expose for manual use (e.g., login handler)
