...
<head>
    ...
    <title>@yield('title','Auth') — {{ \App\Models\Setting::getValue('university_name','University OAS') }}</title>
    <link href="https://fonts.googleapis.com/css2?family=Raleway:ital,wght@0,100..900;1,100..900&display=swap" rel="stylesheet">
    @vite(['resources/css/app.css', 'resources/js/app.js'])
    <style>
        :root{--sand-50:#FBF7EF;--sand-100:#F4ECDC;--coffee-900:#2A1B10;--coffee-700:#4D3422;--terracotta-600:#C2592B;--terracotta-100:#F6E1D3;--line:#E4D7C2;--gold-500:#D4A24C;--danger:#B33A3A;--danger-100:#F6DCDA;}
        *{box-sizing:border-box;}body{margin:0;font-family:'Raleway',sans-serif;background:var(--coffee-900);min-height:100vh;display:flex;align-items:center;justify-content:center;padding:24px;color:#241408;}
        ...
        .mark{width:38px;height:38px;border-radius:10px;background:linear-gradient(155deg,#C2592B,#D4A24C);display:flex;align-items:center;justify-content:center;color:#fff;font-weight:800;}
...
<body>
    ...
    @php $authLogo = \App\Models\Setting::getValue('university_logo'); @endphp
    <a href="{{ route('home') }}" class="brand"><div class="mark" style="overflow:hidden;@if($authLogo)background:#fff;padding:4px;@endif">@if($authLogo)<img src="{{ asset($authLogo) }}" style="width:100%;height:100%;object-fit:contain;" alt="Logo">@else {{ substr(\App\Models\Setting::getValue('university_acronym','UDOM'),0,1) }} @endif</div><div style="text-align:left;line-height:1.1;"><div style="font-weight:700;">{{ \App\Models\Setting::getValue('university_name','University OAS') }}</div><div style="font-size:11px;letter-spacing:.06em;text-transform:uppercase;color:var(--terracotta-100);">Online Admission System</div></div></a>
    <div class="card">
